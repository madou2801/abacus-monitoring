...
function handleLead(req, res) {
  ...
  req.on('end', () => {
    ...
    const lead = {
      ...
      formation: String(payload.formation || '').slice(0, 200),
      modalite: String(payload.modalite || '').slice(0, 50),
    ...
    fs.appendFile(LEADS_FILE, JSON.stringify(lead) + '\n', (err) => {
      ...
      console.log(`[LEAD] ${lead.nom} - ${lead.formation || 'non precise'} (${lead.financement || '?'})`);
      sendJson(res, 200, { ok: true, message: 'Demande enregistree. Un conseiller vous recontacte sous 24h ouvrees.' });
    });
  });
}
...
const server = http.createServer((req, res) => {
  ...
  if (!path.extname(rel)) rel += '.html'; // URLs propres : /financements -> financements.html
